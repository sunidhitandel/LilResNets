...
def get_test_data_loader(config):
    ...
    test_loader = DataLoader(test_dataset, batch_size=int(config['batch_size']/2), shuffle=False, num_workers=16,  pin_memory=torch.cuda.is_available())
    return test_loader
    ...
    def save_samples(dataset, prefix):
        ...
        for class_idx, indices in class_indices.items():
            for i, idx in enumerate(indices[:10]):
                ...
                img_path = os.path.join(output_dir, 'sample_images', f"{prefix}_{class_names[class_idx]}_{i}.png")
